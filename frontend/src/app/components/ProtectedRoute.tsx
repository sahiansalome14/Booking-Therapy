...
export function ProtectedRoute({
	...
	if (requireAuth && user) {
		...
		if (!user.is_profile_complete) {
			if (
				...
				console.log(
					"🛡️ ProtectedRoute: Profile incomplete, redirecting to /seleccionar-rol",
				);
		...
		// Si el perfil ya está completo e intenta entrar a selección de rol
			...
			const dest =
				user.role === "therapist"
					...
					: "/client/dashboard";
			console.log(
				`🛡️ ProtectedRoute: Profile complete, redirecting from role selection to ${dest}`,
			);
	...
	if (!requireAuth && user && user.is_profile_complete) {
		...
		if (location.pathname === "/login") {
			const dest =
				user.role === "therapist"
					...
					: "/client/dashboard";
			console.log(
				`🛡️ ProtectedRoute: Authenticated user on /login, redirecting to ${dest}`,
			);
			return <Navigate to={dest} replace />;
		}
	} else if (
		...
		console.log(
			"🛡️ ProtectedRoute: Authenticated user (no profile) on /login, redirecting to /seleccionar-rol",
		);
		return <Navigate to="/seleccionar-rol" replace />;
	}
