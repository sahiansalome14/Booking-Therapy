...
QUEUE_NAME = "payments_events_queue"
DLQ_NAME = "payments_events_dlq"
PROCESSED_SET = "payments_processed_event_ids"
...
def process_event(event_data):
    """
    Procesa y persiste el evento de sincronización de usuario en la base de datos de Pagos.
    Se ejecuta de manera segura bajo el contexto de la aplicación Flask.
    """
    ...
    # ── Sincronización Transaccional en Base de Datos de Pagos ──
    with app.app_context():
        try:
            ...
            if not user:
                ...
                db.session.add(user)
                db.session.flush()  # Obtener el ID autogenerado del usuario
            else:
                user.first_name = first_name
                user.last_name = last_name

            # Resolver e insertar/actualizar el perfil del usuario (consistencia eventual)
            profile = Profile.query.filter_by(internal_id=internal_id).first()
            if not profile:
                profile = Profile(
...
def run_consumer():
    """
    Inicializa el consumidor, crea las tablas si no existen dentro del app_context y arranca el loop.
    """
    logger.info("Inicializando tablas DDL en la base de datos de Pagos...")
    with app.app_context():
        try:
            db.create_all()
